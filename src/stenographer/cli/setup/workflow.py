...
def _prompt_sound_pack(console: Console, current: str, config_dir: pathlib.Path) -> str:
    ...
    from stenographer.cli.sounds.workflow import parse_sound_pack_choice
    from stenographer.lib.sounds.packs import discover_sound_packs
...
def _audio_devices() -> list[tuple[str, str]]:
    ...
    from stenographer.lib.audio.probe import input_device_choices, query_devices
...
def _automatic_floor(
    ...
    from stenographer.overlay.spectrum.calibration import calibrate_spectrum_profile
...
def _guided_setup(
    ...
    *,
    ...
    console.write()
    console.write(
        doctor.render(caps, config, path, guidance, overlay_guidance=overlay_platform().guidance())
    )
    missing = bool(missing_required(caps))
    if missing:
...
def run(
    *,
    ...
    from stenographer.cli.setup.quick import _quick_wizard
