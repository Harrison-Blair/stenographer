...
The gate itself — :class:`~stenographer.lib.diagnostics.capability_result.Capabilities`,
``REQUIRED``, :func:`~stenographer.lib.diagnostics.capabilities.probe` and
:func:`~stenographer.lib.diagnostics.capabilities.missing_required` — is core
(``stenographer.lib.diagnostics.capabilities``) because the daemon's startup gate shares it.
...
— comes from the platform's :class:`~stenographer.lib.platform.host_guidance.HostGuidance`,
...
from stenographer.cli.doctor.log_status import LogStatus
from stenographer.cli.shared.capabilities import Capabilities
from stenographer.cli.shared.capability_probe import probe
from stenographer.lib.diagnostics.capabilities import REQUIRED, missing_required
...
def format_overlay_status(capability: OverlayCapability, guidance: OverlayGuidance) -> str:
...
def render(
    ...
    *,
    overlay_guidance: OverlayGuidance,
) -> str:
    ...
    *logs* is the already-gathered facts for each log file — :func:`run` does
    ...
    lines.append(f"  overlay: {format_overlay_status(caps.overlay, overlay_guidance)}")
    if logs is not None:
        lines.append("")
        lines.append("logs:")
        for log in logs:
...
def run(cfg: Config, config_path: pathlib.Path) -> int:
    from stenographer.lib.logging.pipeline import log_paths
    from stenographer.lib.platform import current_platform
    from stenographer.overlay.platform import current_platform as overlay_platform
    ...
    print(
        render(
            ...
            cfg,
            config_path,
            current_platform().guidance(),
            logs,
            overlay_guidance=overlay_platform().guidance(),
        )
    )
