...
Assign the first unused species; a species frees for reuse only after **every** worker bearing it has confirmed shutdown (for a pair: both members). **Confirmed shutdown** means the worker is observably gone per your adapter's piping file (absent from the roster / its session ended) — a worker's acknowledgement of a shutdown request is *not* confirmation; a named worker cannot end its own session, so it may ack and then linger idle. If all 18 species are in use (>18 concurrent pairs), append a numeric suffix to the first species — `adelie-2`, then `-3`, applied to both members of the pair — so a full roster never blocks dispatch. Report a one-line roster delta to the user whenever it changes (e.g. `+ fledge-brooder-gentoo, fledge-skua-gentoo → FTHR-007`); give the full roster (name → role → feather) on request. Keep the full name→feather mapping internally — species reuse depends on it.
...
5. **Plumage closeout:** if that was the last unfinished feather of its plumage, first confirm every pair for this plumage's feathers has already been torn down at its green teardown — force-terminate any straggler still present before closing. Then verify each plumage acceptance criterion — citing which feathers and evidence files satisfy it — and present that AC-by-AC accounting through a `confirm-gate` (review). On "Accept", check each box with `fledge criteria check PLM-### <n>` on main, run `fledge status PLM-### fledged`, and commit the spec update. On "Make changes", the gap goes back into the run before the plumage can close.
