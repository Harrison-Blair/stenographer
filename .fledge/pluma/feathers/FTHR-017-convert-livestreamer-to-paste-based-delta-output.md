...
2. **A partial failure can silently break the prefix invariant.** If the regular `wl-copy` succeeds but `--primary` fails, `copy()` returns `False` while the clipboard holds the new text and primary holds the *previous* delta's text. Under the Approach's "`self._typed += text` only on success" bookkeeping, that delta is dropped from `self._typed` but the two selections now disagree. If a later delta then succeeds and pastes, a primary-reading client (kitty pastes from PRIMARY via `paste_from_selection` — see `.fledge/molt/FTHR-015.md`) can deliver a *stale* or *out-of-order* word. The delivered text then contains a gap or a repeat, and is no longer a prefix of the final transcript — a direct violation of PLM-010's never-revised guarantee, presenting as silently wrong text rather than a visible error.

The invariant is the hard constraint, so `_emit()` must not paste when `copy()` returned `False`: on a failed copy, skip the paste and leave `self._typed` unchanged, so the delivered text stops cleanly at a prefix boundary rather than continuing past a gap. Cover this with a test that forces `copy()` to return `False` mid-utterance and asserts (a) no `paste()` fires for that delta, and (b) the reconstructed delivered text is still a prefix of the final transcript. Do not attempt to repair or resynchronise the selections — stopping at a prefix is correct; guessing is not.
