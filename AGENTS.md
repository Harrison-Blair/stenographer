...
6. **Privacy in logs** — numeric/structural metrics and transcript *lengths*
   ...
   `\` escaped, so `key=value` survives a runtime value it never saw; every
   other value stays bare. The rotating `stenographer.log` sink is
