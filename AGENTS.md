...
| `audio.py` | PortAudio recorder: retained pre-negotiated stream, block-copy callback with latest-only handoff to the overlay supervisor, sample-rate fallback + resample, one stale-stream recovery, and a `CaptureStats` for the completed capture. `speech_gate_stats` is the RMS gate: one framing produces both the verdict and the numbers reported beside it, and `speech_gate_passes` is a thin wrapper so the two can never disagree. |
