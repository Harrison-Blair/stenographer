...
hand, once on EACH compositor family:
    ...
    1. Ensure the model is cached (`stenographer-v2 model download`) and you are
       in the `input` group with write access to /dev/uinput.
    2. Start the daemon:  `stenographer-v2 run`   (or the systemd user unit).
...
from stenographer_v2.daemon import (  # noqa: E402
    LOCK_PATH,
