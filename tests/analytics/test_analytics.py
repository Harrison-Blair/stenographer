...
def test_async_writer_persists_cumulative_accepted_recognition_on_cancel(tmp_path):
    path = tmp_path / "analytics.db"
...
def test_real_sqlite_contention_retries_without_double_counting(tmp_path):
    path = tmp_path / "analytics.db"
    store = Store(path)
    store.register_run("seed", {})
    connection = sqlite3.connect(path)
    connection.execute("BEGIN IMMEDIATE")
    session = AnalyticsSession(path, retries=5)
    identity = session.start(1)
    session.finish(identity, "success", {"recognized_words": 8})
    time.sleep(0.15)
    connection.rollback()
    connection.close()
    assert session.close(timeout=3)
    assert store.report()["totals"]["recognized_words"] == 8
    assert len(store.timeline(identity)) == 2


def test_bounded_queue_and_persistent_contention_report_loss(tmp_path):
    path = tmp_path / "analytics.db"
    Store(path).register_run("seed", {})
    connection = sqlite3.connect(path)
    connection.execute("BEGIN IMMEDIATE")
    session = AnalyticsSession(path, queue_size=1, retries=0)
...
def test_numeric_pipeline_checkpoints_preserve_accepted_words_on_delivery_failure(
    ...
    path = tmp_path / "analytics.db"
    session = AnalyticsSession(path, resource_probe=None)
...
def test_run_registration_recovers_after_initial_storage_failure(tmp_path):
    path = tmp_path / "analytics.db"
    Store(path).register_run("seed", {})
    connection = sqlite3.connect(path)
    connection.execute("BEGIN IMMEDIATE")
    session = AnalyticsSession(path, retries=0)
...
def test_negotiated_capture_context_replaces_configured_device_and_survives_finish(tmp_path):
    path = tmp_path / "analytics.db"
    session = AnalyticsSession(path, context={"model": "medium.en", "device": "default"})
    identity = session.start(1)
    session.checkpoint(
        identity,
        "secured_capture",
...
def test_checkpoint_context_rejects_private_fields_before_mutating_snapshot(tmp_path):
    path = tmp_path / "analytics.db"
    session = AnalyticsSession(path)
    identity = session.start(1)
...
def test_queued_completed_utterance_never_profiles_writer_drain(tmp_path):
    path = tmp_path / "analytics.db"
    Store(path).register_run("seed", {})
    connection = sqlite3.connect(path)
    connection.execute("BEGIN IMMEDIATE")
    ...
    session = AnalyticsSession(path, retries=5, resource_probe=probe)
    identity = session.start(1)
    session.finish(identity, "success")
    time.sleep(0.55)
    connection.rollback()
...
def test_probe_completing_after_terminal_is_not_attributed(tmp_path):
    import threading
    ...
    path = tmp_path / "analytics.db"
    session = AnalyticsSession(path, resource_probe=probe)
    identity = session.start(1)
