...
@pytest.mark.parametrize(
    "command,handler", [("doctor", "_cmd_doctor"), ("devices", "_cmd_devices")]
)
...
def test_format_input_devices_marks_default_and_skips_outputs():
    ...
    lines = cli._format_input_devices(devices, default_index=1)
    assert lines == ["* 1: usb mic (1 ch)", "  2: webcam mic (2 ch)"]


def test_format_input_devices_empty():
    assert cli._format_input_devices([{"name": "out", "max_input_channels": 0}], -1) == [
