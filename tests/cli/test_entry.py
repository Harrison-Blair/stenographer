...
import stenographer
from stenographer.cli.devices.handler import _format_input_devices
...
def test_doctor_and_devices_dispatch_to_handlers(command, monkeypatch):
    module = importlib.import_module(f"stenographer.cli.{command}.handler")
    hits = []
...
def test_setup_dispatches_to_handler(monkeypatch):
    from stenographer.cli.setup import handler as setup_command
...
def test_setup_default_dispatches_to_the_noninteractive_writer(monkeypatch):
    ...
    from stenographer.cli.setup import default as default_engine
    from stenographer.cli.setup import workflow as setup_engine

    hits: list[str] = []
    monkeypatch.setattr(default_engine, "write_default", lambda **kw: hits.append("default") or 0)
...
def test_sounds_dispatches_to_handler(monkeypatch):
    from stenographer.cli.sounds import handler as sounds_command
...
def test_main_doctor_dispatches_without_creating_the_daemon_log(monkeypatch, tmp_path):
    ...
    import stenographer.cli.entry as cli
...
def test_main_flushes_the_log_queue_before_returning(monkeypatch, tmp_path, capsys):
    ...
    import stenographer.cli.entry as cli
    from stenographer.lib.logging.pipeline import fmt_event, shutdown_logging
...
def test_with_config_applies_the_configured_log_level(monkeypatch, tmp_path):
    ...
    import stenographer.lib.config.models as config
    import stenographer.lib.config.paths as config_paths
    from stenographer.cli.shared.config import with_config
    from stenographer.lib.logging.pipeline import fmt_event, setup_logging, shutdown_logging

    defaults = config.Config.defaults()
    quiet = dataclasses.replace(
        defaults, feedback=dataclasses.replace(defaults.feedback, log_level="error")
    )
    monkeypatch.setattr(config_paths, "load_or_default", lambda: quiet)
    monkeypatch.delenv("STENOGRAPHER_LOG_LEVEL", raising=False)
...
def test_run_dispatches_config_to_daemon(monkeypatch):
    ...
    import stenographer.cli.run.startup as daemon
    import stenographer.lib.config.models as config
    import stenographer.lib.config.paths as config_paths

    # A real Config: `run` reads feedback.log_level off it before dispatching.
    sentinel = config.Config.defaults()
    monkeypatch.setattr(config_paths, "load_or_default", lambda: sentinel)
    seen = {}
...
def test_transcribe_dispatches_to_handler(monkeypatch):
    ...
    from stenographer.cli.transcribe import handler as transcribe_command
...
def test_model_download_dispatches_to_handler(monkeypatch):
    ...
    from stenographer.cli.model.download import handler as model_command
