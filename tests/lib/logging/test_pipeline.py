...
def test_setup_wires_the_utterance_stamp_onto_module_loggers(tmp_path):
    ...
    try:
        set_utterance(7)
        logging.getLogger("stenographer.lib.daemon").info(fmt_event("pipeline", "started"))
    finally:
        set_utterance(None)
        shutdown_logging()
...
def test_log_failure_unsafe_never_renders_the_exception_message(captured):
    ...
    assert frames.startswith("test_pipeline.py:")
...
def test_child_relay_preserves_stamp_and_drains_before_parent_listener(tmp_path):
    ...
    from stenographer.lib.logging.pipeline import forward_worker_record, start_worker_log_relay
